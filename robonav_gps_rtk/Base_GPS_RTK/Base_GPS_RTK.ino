...
// --- GPS (HardwareSerial) ---
const byte rxPin = 36;  // ESP32-devKit = 16, ESP32-PoE-IND-ISO = 36
const byte txPin = 4;   // ESP32-devKit = 17, ESP32-PoE-IND-ISO = 4
...
const char* udpAddress = "255.255.255.255";  // broadcast IP

uint16_t bufferIndex = 0;
uint8_t rawBuffer[MAX_BUFFER+1];
int lastNbConnected = -1;
...
{
  Serial.begin(921600);
  Serial.println("Start BASE-RTK GPS module");

  // Init GPS
  Serial1.begin(115200, SERIAL_8N1, rxPin, txPin);
  resetGPS();
  // ESP in transparent Mode relie GPS to Serial pour le debug avec GNSS
  transparentMode( false );
  // Configurer GPS en mode Base RTK
  configureGPS_BaseRTK( 1, 0,0,0 );
...
void loop()
{
  ...
  while( Serial1.available() )
  {
    ...
    if( bufferIndex == 1 ) {
        ...
        if( !receivingRTCM && !receivingNMEA )
            bufferIndex = 0;
        continue;
    }
    // Détection de fin de trame
    else if( (bufferIndex >= MAX_BUFFER) ||
             (receivingNMEA && bufferIndex >= 2 && rawBuffer[bufferIndex-2] == 0x0D && rawBuffer[bufferIndex-1] == 0x0A ) ||
  ...
  if( Serial.available() ) {
    ...
    Serial.println(mesg);
    Serial1.println(mesg);  // Transmet au GPS
  }
...
void resetGPS()
{    // Reboot Module
    Serial1.println("$PAIR023*3B");
    delay( 2000 );      // Force wait for module rest complete
}

void transparentMode( bool mode )
{
  if( mode ) {
    // Transparent Mode
    while( true ) {
...
void configureGPS_BaseRTK( int mode, double x, double y, double z )
{
  // Base Station - Config Mode
  Serial1.println("$PQTMRESTOREPAR*13");            // Restore parametres par défaut
  Serial1.println("$PQTMCFGRCVRMODE,W,2*29");       // Configure en mode Base RTK
  Serial1.println("$PQTMSAVEPAR*5A");               // Sauvegarde de la configuration
  resetGPS();

  // Survey-in (Mode 1) or Fixed ECEF Base (Mode 2) -- Config Mode
  if( mode == 1 )
      Serial1.println("$PQTMCFGSVIN,W,1,300,2,0,0,0*23"); // Set the base Station X,Y,Z coordinnates
  else if( mode == 2 ) {
      //Serial1.println("$PQTMCFGSVIN,W,2,0,0,4333751.903,-113958.145,4662601.388*29"); // Set the base Station X,Y,Z coordinnates
    initBaseECEFCoord( x, y, z );
  }
  Serial1.println("$PQTMCFGNMEADP,W,3,8,3,3,3,3*39"); // Set decimal precision for NMEA
  Serial1.println("$PQTMSAVEPAR*5A");               // Sauvegarde de la configuration
  resetGPS();

  // GPS Parameters Config
  Serial1.println("$PAIR432,1*22");                 // Output RTCM3 MSM7
  Serial1.println("$PAIR434,1*24");                 // Output RTCM3 antenna position (1005)
  Serial1.println("$PAIR436,1*26");                 // Output RTCM3 with satellites ephemerides
  Serial1.println("$PAIR062,0,01*0F");              // Enable GCA NMEA Messages
  Serial1.println("$PQTMSAVEPAR*5A");               // Sauvegarde de la configuration

  // Message that are not config saved
  if( mode == 1 )
    Serial1.println("$PQTMCFGMSGRATE,W,PQTMSVINSTATUS,1,1*58");  // Survey IN status - ON
  else
    Serial1.println("$PQTMCFGMSGRATE,W,PQTMSVINSTATUS,0,1*59");  // Survey IN status - OFF
}


bool invalidFrame( const char *err_msg, const char *buffer ) {
  if( true ) // DEBUG_GPS_INFO )
  { // Debug Print Buffer
...
bool parseNMEA( const uint8_t *rtcmBuffer, size_t length ) {
  ...
  if( checkNMEACRC( nmea ) )
  {
    ...
    if( nmea.startsWith("$GNGGA") ) {
      ...
      sendUDP( rtcmBuffer, length );                  // Inform Rover for Survey complete
  ...
  if( (nmea.startsWith("$GNGGA")) || (nmea.startsWith("$GNRMC")) ) {
    ...
    if( nmea.startsWith("$GNGGA") && fields[fixIndex].length() > 0 ) {
      ...
      switch (code) {
        ...
        default: mesg += String(code) + ",Fix unknown"; break;
      }
    }
    ...
    if( fields[latIndex].length() > 0 && fields[lonIndex].length() > 0 ) {
      ...
      // Insert Lat,Lon:
      mesg += "," + String(latitude, 10) + ","  + String(longitude, 10);
    }
    
    // Output GPS position data
    Serial.println( mesg );
    sendUDP( (const unsigned char*)mesg.c_str(), mesg.length() );
  }
}
...
void surveyIn_ECEFCoordinate( String nmea )
{
  ...
  // Le Le Survey-In est terminé => Ecriture des coord en mode 2 (Write ECEF)
  if( obs == duration ) {
    configureGPS_BaseRTK( 2, x,y,z );
  }
}

void initBaseECEFCoord(double x, double y, double z)
{ 
  char nmeaBuffer[100];
...
{ 
  ...
  { 
    ...
    if( crcRx == crcCalc )
    {
      ...
      // Envoi de la trame RTCM de la base RTK -> vers les Rovers RTK
      Serial.print('.');
      sendUDP( rtcmBuffer, length );
      return true;
    }
    else {
