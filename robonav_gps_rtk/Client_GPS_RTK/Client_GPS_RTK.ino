...
uint8_t incomingPacket[MAX_BUFFER+1];  // Buffer de réception
...
void setup()
{
  ...
  Serial.println("Start Rover-RTK module");

  // Configuration de GPS en mode Rover RTK
  delay(2000);
  Serial1.println("$PQTMRESTOREPAR*13");            // Restore parametres par défaut
  Serial1.println("$PQTMCFGRCVRMODE,W,1*2A");       // Configure en mode Rover RTK
  Serial1.println("$PQTMSAVEPAR*5A");               // Sauvegarde de la configuration
  Serial1.println("$PAIR023*3B");                   // Reboot Module
  delay(2000);
  Serial1.println("$PAIR062,1,0*3C");               // Turn off GLL messages
  Serial1.println("$PAIR062,2,0*3C");               // Turn off GSA messages
  Serial1.println("$PAIR062,3,0*3D");               // Turn off GSV messages
  Serial1.println("$PAIR062,5,0*3B");               // Turn off VTG messages
  Serial1.println("$PAIR062,0,01*0F");              // Enable GCA NMEA Messages

  //Serial1.println("$PAIR100,1,0*3A");
  //Serial1.println("$PAIR400,1*23");
  Serial1.println("$PQTMCFGNMEADP,W,3,8,3,3,3,3*39"); // Set decimal precision for NMEA
  //Serial1.println("$PAIR050,200*21");              // Set ouput interval to 200ms
  Serial1.println("$PQTMSAVEPAR*5A");               // Sauvegarde de la configuration
...
{ 
  ...
  if( packetSize ) {
    ...
    if( len >= 6 ) {
      ...
      if( incomingPacket[0] == '$' )
      {   // NMEA Message
          String nmea = String(incomingPacket, len);
          Serial.println();
          Serial.println( nmea );
      }
      else  // RTCM3 message
      {
        validFrame = parseRTCM3( incomingPacket, len );
        if( !validFrame )
            Serial.print('x');

/*      Serial.println( len );
      Serial.print(" Message RTCM : ");
