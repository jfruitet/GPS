...
*/
  ...
  Serial2.println("$PQTMRESTOREPAR*13");            // Restore parametres par défaut
  Serial2.println("$PQTMCFGRCVRMODE,W,2*29");       // Configure en mode Base RTK
  Serial2.println("$PQTMSAVEPAR*5A");               // Sauvegarde de la configuration
  Serial2.println("$PAIR023*3B");                   // Reboot Module
  delay(2000);
  Serial2.println("$PAIR432,1*22");                 // Output RTCM3 MSM7
  Serial2.println("$PAIR434,1*24");                 // Output RTCM3 antenna position (1005)
  Serial2.println("$PAIR436,1*26");                 // Output RTCM3 with satellites ephemerides
  ...
  Serial2.println("$PQTMCFGSVIN,W,2,0,0,4331619.128,-112346.724,4664594.792*24"); // Set the base Station X,Y,Z coordinnates
  Serial2.println("$PQTMSAVEPAR*5A");               // Sauvegarde de la configuration
      ...
      if( DEBUG_GPS_INFO )
      {
        ...
        switch (msgType) {
          case 1005:
            ...
            break;
          case 1077:
            Serial.println("RTCM GNSS (1077) - non décodé ici");
            break;
          default:
