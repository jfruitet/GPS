...
20250522
BASE Ublox
Code implantant un DGPS pour Ublox M8N
Repris par JF à partir du code ./dev/GPS/src_icam/DGPS M8N/Base_GPS.ino Développé par Agathe D et Thed K

Modification de l'algorithme de mesure de la position GPS
La fréquence est déterminée par le module Ublox NEO M8N qui est fixée à 1 hertz (une mesure par seconde).
100 mesure sont effectuées (100 secondes) ; la moyenne, le mode et la médiane sont calculé.e.s
Les valeurs de delta avec une position fixe est diffusée pour être traitée par le rover (bouée)

A PROGRAMMER
Pour le rover le code est similaire ; les deltas reçus sont appliqués à la moyenne de 100 positions reçues.

20250605
Cet algorithme n'a pas été implanté faute de disposer de GPS fiables. En effet deux valeurs successives 
sont espacées de 20cm et 100 valeurs montrent un déplacement erratique. Deux GPS identiques n'ont pas les mêmes errements
ce qui rend illusoir un DGPS.  
**/
...
// --- Base Position ---
// Cette position doit être fournie lors de la séquence d'initialisation
// ou lue dans un fichier JSON
// ou transmise par WiFi
double base_lat = 472751330;
double base_lon = -15065300;
...
// GPS latitude et longitude in classical double representation
double gpsLat;
double gpsLon;

// Valeurs de réfence obtenues lors de la première activation de la balise
double gpsAvLat0;
double gpsAvLon0;
double gpsMedLat0;
double gpsMedLon0;
...
// GPS latitude et longitude moyenne, mode et médiane sur n valeurs dans la représentation habituelle
// Les valeurs mode et médiane sont aussi sont moyennées
// la fonction filtrerGPS() modifie cette structure
struct gpsLonLat { 
  double avLon;
  double avLat;
  double modLon;
...
{ 
  ...
  if( Serial.available() > 0 ) {
    ...
    Serial.println("UBX-GPS Reset and setup PORT configuration");
  ...
  // Première valeur reçue. Affiche les informations attachées
  debugGPS();
  
  // Moyenne, mode et médiane sur 100 valeurs
  
  if (filtrerGPS(100) && premiereValeurGPS) { // stocker 
    double gpsAvLat0 = gpsLonLat.avLat;
    double gpsAvLon0 = gpsLonLat.avLon;
    double gpsMedLat0 = gpsLonLat.medLat;
    double gpsMedLon0 = gpsLonLat.medLon;
    double gpsModLat0 = gpsLonLat.modLat;
    double gpsModLon0 = gpsLonLat.modLon;
    premiereValeurGPS=0;
  }
  else {
    // Relancer la réception
    if (filtrerGPS(100)){
      double gpsAvLat1 = gpsLonLat.avLat;
      double gpsAvLon1 = gpsLonLat.avLon;
      double gpsMedLat1 = gpsLonLat.medLat;
      double gpsMedLon1 = gpsLonLat.medLon;
      double gpsModLat1 = gpsLonLat.modLat;
      double gpsModLon1 = gpsLonLat.modLon;

      // calculer les deltas avec une position connue  et diffuser par WiFi / UDP
      printDataGPS(base_lat, base_lon, gpsAvLat1, gpsAvLon1); // Fonction à compléter car pour le moment elle ne traite que la moyenne.
      // Calculer les deltas avec la première position enregistrée
      // C'est le calcul à effectuer sur le rover pour déterminer si celui-ci s'est ddéplacé...
      printDeltaGPS(gpsAvLat0, gpsAvLon0, gpsAvLat1, gpsAvLon1);
...
// Appel unique après initialisation du GPS
void debugGPS (){
    if( config_gps_ok ) {
      ...
      if ( msgType == MT_NAV_STATUS ) {
        ...
        Serial.print(" lat/lon: "); Serial.print(ubxMessage.navPvt.lat); Serial.print(","); Serial.print(ubxMessage.navPvt.lon);
...
// Call n times processGPS()
// Compute min, max, average, mode and median
// Modifie par effet de bord la structure ad hoc 
int filtrerGPS(int n) {

  double gps_lat = 0.0;
  double gps_lon = 0.0;

  double gps_average_lat = 0.0;
  double gps_average_lon = 0.0;

  double gps_mode_lat = 0.0;
  double gps_mode_lon = 0.0;

  double gps_median_lat = 0.0;
  double gps_median_lon = 0.0;

  double gps_min_lat = 0.0;
  double gps_min_lon = 0.0;

  double gps_max_lat = 0.0;
  double gps_max_lon = 0.0;

  double t_gps_lat[];
  double t_gps_lon[];

// histogramme des positions 
  int index_gps_lat[]; // Indirection qui associe à chaque valeur de lat/lon une case dans l'histogramme des occurrences
  int index_gps_lon[];

  int h_gps_lat[]; // Table des occurrences pour les valeurs lat/lon regroupées dans des cases de largeur "ecart_lat" / "ecart_lon"
  int h_gps_lon[];
  ...
  while ((i < n) && (compteur < 1000)) { // Sortir après 1000 tentatives.
    if ( config_gps_ok ) {

      int msgType = processGPS();
      if ((msgType == MT_NAV_POSLLH ) || ( msgType == MT_NAV_PVT )){
        if ( msgType == MT_NAV_POSLLH ) {
          gps_lat = ubxMessage.navPosllh.lat;
          gps_lon = ubxMessage.navPosllh.lon;
        }
        else {
          gps_lat = ubxMessage.navPvt.lat;
          gps_lon = ubxMessage.navPvt.lon;     
        }
  ...
  if (i>10){  // Ordonner les tableaux si 10 valeurs au moins enregistrées
    qsort(t_gps_lat, sizeof t_gps_lat / sizeof *t_gps_lat,  sizeof *t_gps_lat);
    qsort(t_gps_lon, sizeof t_gps_lon / sizeof *t_gps_lon,  sizeof *t_gps_lon);

    gps_min_lat = t_gps_lat[0];  
    gps_min_lon = t_gps_lon[0];

    gps_max_lat = t_gps_lat[i-1];  
    gps_max_lon = t_gps_lon[i-1];

    // Largeur d'une case (bucket) en latitude et longitude
    double ecart_lat = (gps_max_lat - gps_min_lat) / (double)i;
    double ecart_lon = (gps_max_lon - gps_min_lon) / (double)i;
    
    // Calcul du mode
    // Les valeurs sont disposées dans un tableau d'occurrences à une position déterminée par round((t_gps_lat[j] - gps_min_lat) / ecart_lat);
    ...
    // Histogramme : enregistre le nombre d'occurrences dans une case donnée.
    j=0;
    while (j<i){
      index = round((t_gps_lat[j] - gps_min_lat) / ecart_lat);
      index_lat[j] = index;
      h_gps_lat[index]++;

      index = round((t_gps_lon[j] - gps_min_lon) / ecart_lon);
      index_lon[j] = index;
      h_gps_lon[index]++;
      j++;
    }

    // Recherche de l'index du mode
    // Si la distribution est multi modale on ne garde que le premier mode.
    // Cela pourrait-il être amélioré ?
    int index_max_lat=0;
    int index_max_lon=0;
    j=0;
    while (j<i){  
      if (h_gps_lat[j]>index_max_lat){
        index_max_lat = j;       
      }
      if (h_gps_lon[j]>index_max_lon){
        index_max_lon = j;        
      }    
      j++;  
    }

    // Calcul de la valeur moyenne pour toute la distribution
    j=0;
    while (j<i){
      gps_average_lat += t_gps_lat[j];
      gps_average_lon += t_gps_lon[j];
      j++;
    }
    
    gps_average_lat /= (double)i;
    gps_average_lon /= (double)i;
    
    // Calcul de la valeur moyenne pour les valeurs contenues dans la case du mode
    j=0;
    while (j<i){
      if (index_lat[j] == gps_average_lat){
        gps_mode_lat+=t_gps_lat[j];
      }
      if (index_lon[j] == gps_average_lon){
        gps_mode_lon+=t_gps_lon[j];
      }      
      j++;
    }

    gps_mode_lat /= (double) h_gps_lat[gps_average_lat];
    gps_mode_lon /= (double) h_gps_lon[gps_average_lon];    
  
    // Calcul de la médiane
    int index_med_lat=0;
    int index_med_lon=0;    
    if (i%2==0){
      gps_median_lat = t_gps_lat[i/2];
      gps_median_lon = t_gps_lon[i/2];  
    }
    else {
      gps_median_lat = t_gps_lat[(i+1)/2];
      gps_median_lon = t_gps_lon[(i+1)/2];  
    }
 
    // Calcul de la valeur moyenne des valeurs proches de la médiane dans un écart 
    if (i%2==0){
      gps_median_lat = t_gps_lat[i/2];
      gps_median_lon = t_gps_lon[i/2];  
    }
    ...
    j=0;
    double average_mediane_lat =0.0;
    double average_mediane_lon =0.0;
    int k_lat=0;
    int k_lon=0;
    while (j<i){
      if ((t_gps_lat[j] >= (gps_median_lat - (ecart_lat / 2.0))) && (t_gps_lat[j] >= (gps_median_lat + (ecart_lat / 2.0)))){
        average_mediane_lat+=t_gps_lat[j];
        k_lat++;
      }
      if ((t_gps_lon[j] >= (gps_median_lon - (ecart_lon / 2.0))) && (t_gps_lon[j] >= (gps_median_lon + (ecart_lon / 2.0)))){
        average_mediane_lon+=t_gps_lon[j];
        k_lon++;
      }
    }
    if ((k_lat>0) && (k_lon>0)){
      average_mediane_lat/=(double)k_lat;
      average_mediane_lon/=(double)k_lon;
    }

    // GPS latitude et longitude in classical double representation
    gpsLonLat.avLon = gps_average_lon;
    gpsLonLat.avLat = gps_average_lat;

    gpsLonLat.modLon = gps_mode_lon;
    gpsLonLat.modLat = gps_mode_lat;

    gpsLonLat.medLon = average_mediane_lat;
    gpsLonLat.medLat = average_mediane_lon; 
  }

  // Si moins de 10 valeurs le filtrage a échoué
  return (i>10);

}

// Calcule le delta entre une position fixe et une position reçue
// La position fixe peut être la valeur d'initialisation des 100 premières mesures. 
void printDeltaGPS (double base_lat, double base_lon, double lat, double lon){
  ...
  if (gpsLonLat){
        ...
        double delta_lat = base_lat - lat;
        double delta_lon = base_lon - lon;
